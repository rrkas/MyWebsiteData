...
for filename in filenames:
    if filename.endswith(".csv"):
        ...
        if gdrive_file_id in df.columns:
            file_ids.extend(list(df[gdrive_file_id]))
            total_record_count = len(file_ids)
            unique_record_count = len(set(file_ids))
            ...
            invalid_file_ids = list(
                filter(lambda x: len(x) != len_file_id, df[gdrive_file_id])
            )
